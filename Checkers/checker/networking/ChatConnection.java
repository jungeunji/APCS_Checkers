...
/**
 * ChatConnectionHandler is in charge of listening for connections from
 * and initating connections to remote hosts.  Because it must wait for
 * new connections, the class is designed as a thread, with a main loop
 * that simply blocks while waiting for new connections to arrive.
 *
 * The class also contains methods to initiate outgoing connections in
 * response to commands from the main class.
 *
 * Once a socket has been created (either receving or outgoing), this
 * class attaches two new threads to it: one for reading data from the
 * socket, and one for writing new data out to it.  These threads are
 * implemented using the ChatSender and ChatReceiver classes.
 *
...
public class ChatConnectionHandler extends Thread
{
    ...
    protected Map<SocketName, ChatSender> senders;

    /** Collection of receiving threads */
    protected Map<SocketName, ChatReceiver> receivers;
     ...
     * are created.
     * 
     * @param cd  The display object to pass on to child threads
     * @param p   The port to bind to and listen on
     */
    public ChatConnectionHandler( ChatDisplay cd, int p )
    {
        super( "ChatConnectionHandler-" + p );

        senders = new HashMap<SocketName, ChatSender>();
        receivers = new HashMap<SocketName, ChatReceiver>();
     ...
     * connections).
     * 
     * @param sock  The Socket to spawn listeners for
     * 
     ...
     * 
     * @param name  The name and connection parameters of the socket to create
     */
     ...
     * 
     * @param name The name and connection parameters of the socket to destroy
     */
    public void disconnect( SocketName name )
    {
        System.out.println( "Disconnecting from '" + name + "'" );

        ChatSender cs = senders.remove( name );
        ChatReceiver cr = receivers.remove( name );
    ...
    public void send( String s )
    {
        ...
        while ( iter.hasNext() )
        {
            ChatSender cs = senders.get( iter.next() );
            cs.send( s );
        }
    }
